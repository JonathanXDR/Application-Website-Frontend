...
// documentation's blanket claim that list endpoints paginate. Two types stop
// a caller from reading `has_more` off a response that never carries it and
// treating the `undefined` as "no more pages".
...
// Normalized pagination input, already clamped by the time a handler sees
// it.
//
...
// index signature, and TypeScript grants an implicit one only to a type
// alias.
...
// FastAPI's own error shape, which answers a request that never reaches
// Flick's beta router (observed on `GET /api/beta` with no trailing slash).
// Every path this client builds begins with a slash, so it exists only so
