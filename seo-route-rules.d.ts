 ...
 * skipped and the Nuxt config type system cannot see the `robots` route rule.
 * This shim mirrors the augmentation so the type is accepted in routeRules.
 *
 * TODO: remove once @nuxtjs/robots widens its compat check to `>= 4`,
 *       or once we downgrade `future.compatibilityVersion`.
 *
