        ...
        >
          ...
          >
            <NavigatorCardItem
              ...
              :enable-focus="!externalFocusChange"
        ...
        <div class="navigator-filter">
          <div class="input-wrapper">
            <FilterInput
              ...
              :should-truncate-tags="shouldTruncateTags"
...
export default {
  ...
  mixins: [keyboardNavigation, filteredChildrenMixin, tagsProvider],
  props: {
    technologyPath: {
      type: String,
      default: '',
  ...
  computed: {
    ...
    politeAriaLive() {
      ...
      if (!hasNodes) return ''
      return this.$tc(ITEMS_FOUND, navigatorItems.length, {
      ...
      for (let i = childrenLength; i >= 0; i -= 1) {
        ...
        if (child.childUIDs.length) {
          ...
          allChildren = removeDeprecated(
            getAllChildren(child.uid, childrenMap),
            deprecatedHidden,
          )
        }
    ...
    hasFilter({ debouncedFilter, selectedTags, apiChanges }) {
      return Boolean(
        debouncedFilter.length || selectedTags.length || apiChanges,
      )
    },
    /**
  ...
  watch: {
    ...
    apiChanges(value) {
      ...
      this.selectedTags = this.selectedTags.filter(
        t => !Object.values(CHANGES_TAGS).includes(t),
      )
    },
  ...
  methods: {
    ...
    toggleAllNodes() {
      const parentNodes = this.children.filter(
        child =>
          child.parent === INDEX_ROOT_KEY
          && child.type !== TopicTypes.groupMarker
          && child.childUIDs.length,
      )
    ...
    toggle(node) {
      ...
      if (isOpen) {
        ...
        this.setUnlessEqual('openNodes', {
          ...this.openNodes,
          [node.uid]: true,
        })
        // include all childUIDs to get opened
        include = getChildren(node.uid, this.childrenMap, this.children).filter(
          child => this.renderableChildNodesMap[child.uid],
        )
      }
    ...
    augmentRenderNodes({ uid, include = [], exclude = [] }) {
      ...
      if (include.length) {
        // remove duplicates
        const duplicatesRemoved = include.filter(
          i => !this.nodesToRender.includes(i),
        )
        // clone the nodes
    ...
    async scrollToElement() {
      ...
      const index = this.nodesToRender.findIndex(
        child => child.uid === this.activeUID,
      )
    ...
    getChildPositionInScroller(element) {
      ...
      const offset = {
        top: Number.parseInt(paddingTop, 10) || 0,
        bottom: Number.parseInt(paddingBottom, 10) || 0,
      }
      // get the position of the scroller in the screen
      const { y: areaY, height: areaHeight }
    ...
    handleActivePathChange(activePath) {
      ...
      if (currentActiveItem) {
        ...
        const matchingItem = [...children, ...siblings, ...parents].find(
          child => child.path === lastActivePathItem,
        )
    ...
    focusNodeParent(item) {
      ...
      if (!parent) return
      const parentIndex = this.nodesToRender.findIndex(
        c => c.uid === parent.uid,
      )
...
.navigator-filter {
  ...
  @include safe-area-left-set(
    padding-left,
    var(--nav-filter-horizontal-padding)
  );
  @include safe-area-right-set(
    padding-right,
    var(--nav-filter-horizontal-padding)
  );
