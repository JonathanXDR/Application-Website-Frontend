...
export default defineNuxtConfig({
  ...
  hooks: {
    ...
    // Workaround for issue #36: the prerenderer writes correct HTML to
    // de/index.html, but nuxt-ai-ready also writes the Markdown twin to
    // de/index.md in the same directory, and Vercel then resolves the canonical
    // /de/ request to the Markdown twin, serving raw text. Relocate every
    // <dir>/index.md twin to a sibling <dir>.md after the Vercel output is built,
    // which removes the directory-index collision (so /de/ resolves to the HTML)
    // and, as a bonus, aligns the twin with its own `<link rel="alternate">`
    // target (/de.md). Runs in the final build's close hook, after .vercel/output
    // exists. Drop once nuxt-ai-ready writes twins outside the page directory.
    'nitro:init'(nitro) {
      nitro.hooks.hook('close', async () => {
        const fs = await import('node:fs')
        const { join } = await import('node:path')
        const cwd = process.cwd()
        const root = `${cwd}/.vercel/output/static`
        let moved = 0
        const walk = (dir: string) => {
          ...
          for (const entry of entries) {
            ...
            else if (entry.name === 'index.md' && dir !== root) {
              fs.renameSync(full, `${dir}.md`)
              moved += 1
            }
          }
        }
        ...
        // The `<locale>/index.html => <locale>/` overrides are shadowed on Vercel
        // (the .md twin was winning), so drop them and let standard directory
        // index resolution serve the HTML files at their clean URLs.
        let removed = 0
        try {
          const cfgPath = `${cwd}/.vercel/output/config.json`
          ...
          console.log(
            '[md-relocate] config rewrite skipped:',
        ...
        try {
          canonicalHead = fs
            .readFileSync(`${root}/de/index.html`, 'utf8')
            .slice(0, 20)
        }
        catch {
          canonicalHead = 'UNREADABLE'
        }

        console.log(
          `[md-relocate] moved=${moved} overridesRemoved=${removed} `
          + `de/index.html starts="${canonicalHead}"`,
        )
      })
    },
